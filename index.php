...
$res = mysqli_select_db("$mysql_base");
if (!$res)
{
...
$res = @mysqli_query("DESC $mysql_table");
if ( mysqli_errno() != 0 )   //$mysql_table autocreating
{
    mysqli_query("CREATE TABLE $mysql_table (
...
function NewMsg (days,lang) {
    ...
    w = window.open ("newmsg.php?id=0&days=" + days + "&lang=" + lang, winname, "<? echo $js_window_params ?>");
    w.focus();
    return "#";
}
...
<p><a href="newmsg.php?id=0&days=<? echo $days; ?>&lang=<? echo $lang; ?>" OnClick="this.href=NewMsg(<?echo "$days,'$lang'"?>);" <?mouse_text($msg["new_thread"])?>><?echo $msg["new_thread"]?></a>
<br><br>
